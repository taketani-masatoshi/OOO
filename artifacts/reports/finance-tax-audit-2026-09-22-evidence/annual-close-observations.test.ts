...
describe("annual close acceptance", () => {
  ...
  it("AUDIT A2: retry after opening switch but before committed state fails", () => {
    lockPreparedYear();
    const first = closeAccountingYear({fiscalYear: FY, operatorId: OPERATOR});
    expect(first.ok).toBe(true);
    const statePath = annualCloseTransactionPath(FY);
    ...
    console.log("AUDIT A2", JSON.stringify({ok:retry.ok, errors:retry.evaluation.errors}));
    expect(retry.ok).toBe(false);
    expect(parseYaml(readFileSync(statePath,"utf8")).phase).toBe("committing");
  });
  it("AUDIT A3: bank state changes after locks are not rechecked by annual close", () => {
    lockPreparedYear();
    const bankPath = join(getDataDir(), "finance/bank-statements.yaml");
    const bank = parseYaml(readFileSync(bankPath,"utf8"));
    bank.entries = bank.entries.map((row: object) => ({category:"other",description:"synthetic",account_id:"BANK-001",...row}));
    writeFileSync(bankPath, JSON.stringify(bank));
    const imported = importBankStatementCsvText({csvText:"date,direction,amount,category,description,account_id,reference,counterparty\n2026-02-20,inflow,999,other,synthetic,BANK-001,AUDIT-NEW,SYNTHETIC\n"});
    console.log("AUDIT A3 import", JSON.stringify(imported));
    const evaluation = evaluateAnnualCloseGates(FY);
    console.log("AUDIT A3", JSON.stringify({can_close:evaluation.can_close, errors:evaluation.errors}));
    expect(evaluation.can_close).toBe(true);
  });
