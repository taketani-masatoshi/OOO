...
**Canonical edit path:** `Web/` (paths here are repo-relative — `git rev-parse --show-toplevel`)
...
| Role | Path |
|------|------|
| **Canonical edit / daily work** | `Web/` |
| Publish mirror (optional) | `Community/sites/coming-soon` |
| Legacy source (do not prefer) | legacy `OS_Community/sites/coming-soon` checkout (see `archive/MIGRATION-REPORT.md`) |
| Provisional canvas-web branding | `extracts/provisional-canvas-web/` (not production, extract only) |
